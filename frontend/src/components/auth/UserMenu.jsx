...
export default function UserMenu() {
  ...
  const setSecurityDialogOpen = useAuthStore((s) => s.setSecurityDialogOpen);
    ...
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 bg-slate-700 rounded shadow-xl border border-slate-600 z-50 min-w-[180px] py-1">
          ...
          <button
            onClick={() => { setSecurityDialogOpen(true); setOpen(false); }}
            className="block w-full text-left px-4 py-2 text-sm hover:bg-slate-600 transition-colors flex items-center gap-2"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
