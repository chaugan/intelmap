...
export default function TacticalMap() {
  ...
  return (
    <div className="absolute inset-0" data-map-container>
      ...
      {/* Fly-around rotation button - visible when pitched, right of compass */}
      {pitch > 5 && (
        <div className="absolute top-4 left-[4.5rem] z-10 flex items-center gap-1">
          {/* Slow down button - only visible when rotating */}
          {rotating && (
            <button
              onClick={() => setRotationSpeed(Math.max(2, rotationSpeed - 2))}
              className="w-8 h-8 rounded-full bg-slate-800/80 hover:bg-slate-700/90 flex items-center justify-center shadow-lg transition-colors text-white font-bold text-lg"
          ...
          {/* Speed up button - only visible when rotating */}
          {rotating && (
            <button
              onClick={() => setRotationSpeed(Math.min(32, rotationSpeed + 2))}
              className="w-8 h-8 rounded-full bg-slate-800/80 hover:bg-slate-700/90 flex items-center justify-center shadow-lg transition-colors text-white font-bold text-lg"
