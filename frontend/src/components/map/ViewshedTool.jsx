...
export default function ViewshedTool() {
  ...
  const saveToProject = () => {
    ...
    if (toolMode === 'horizon' && horizonResult) {
      socket.emit('client:viewshed:save', { projectId: activeProjectId, layerId: activeLayerId || null, type: 'horizon', longitude: observer.lng, latitude: observer.lat, observerHeight: 0, radiusKm: horizonRadiusKm, geojson: { properties: { horizonProfile: horizonResult.horizonProfile } }, stats: horizonResult.stats });
    } else if (result) {
      socket.emit('client:viewshed:save', { projectId: activeProjectId, layerId: activeLayerId || null, type: 'viewshed', longitude: observer.lng, latitude: observer.lat, observerHeight, radiusKm, geojson: result.geojson, stats: result.stats });
    } else {
      return;
    }
    reset();
          ...
          <div className="space-y-1">
            ...
            <div className="space-y-px max-h-32 overflow-y-auto">
              {savedItems.map((v) => {
                ...
                const label = isHorizon ? `${isNo ? 'Horisont' : 'Horizon'} ${v.radiusKm || ''}km` : `${isNo ? 'Sikt' : 'Viewshed'} ${v.radiusKm ? Math.round(v.radiusKm * 10) / 10 : ''}km`;
                return (
                  <div key={v.id} className={`flex items-center gap-1.5 text-[11px] rounded px-1 py-0.5 hover:bg-slate-700/50 ${isVis ? '' : 'opacity-40'}`}>
                    <button
                      ...
                      className={`shrink-0 ${isVis ? (isHorizon ? 'text-purple-400' : 'text-red-400') : 'text-slate-600'}`}
